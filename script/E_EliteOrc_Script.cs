...
public class E_EliteOrc_Script : MonoBehaviour, IDamageable
{
    public int maxHP = 90;
    ...
    private enum EliteOrcState { Walk, Attack, Damaged, Die }
    private EliteOrcState currentState;
    ...
    void Start()
    {
        ...
        SetState(EliteOrcState.Walk);
    }

    void Update()
    {
        if (currentState == EliteOrcState.Walk)
        {
    ...
    {
        if (currentState == EliteOrcState.Die) return;

        if (collision.CompareTag("Arrow"))
        {
            if (currentState != EliteOrcState.Die)
            {
    ...
    IEnumerator AttackAfterAnimation()
    {
        ...
        yield return new WaitUntil(() =>
            animator.GetCurrentAnimatorStateInfo(0).IsName("Enemy_EliteOrc_Attack1") ||
            animator.GetCurrentAnimatorStateInfo(0).IsName("Enemy_EliteOrc_Attack2"));
    ...
    public void TakeDamage(int damage)
    {
        ...
        if (currentHP <= 0)
        {
            SetState(EliteOrcState.Die);
            StartCoroutine(DieAfterAnimation());
        }
        else
        {
            SetState(EliteOrcState.Damaged);
    ...
    void SetState(EliteOrcState newState)
    {
        ...
        switch (currentState)
        {
            case EliteOrcState.Walk:
                animator.Play("Enemy_EliteOrc_Walk");
                break;
            case EliteOrcState.Attack:
                attackCount = (attackCount % 3) + 1; // 弱 → 弱 → 強
                if (attackCount == 3)
                {
                    animator.Play("Enemy_EliteOrc_Attack2"); // 強攻撃
                }
                else
                {
                    animator.Play("Enemy_EliteOrc_Attack1"); // 弱攻撃
                }
                break;
            case EliteOrcState.Damaged:
                animator.Play("Enemy_EliteOrc_Damaged");
                break;
            case EliteOrcState.Die:
                animator.Play("Enemy_EliteOrc_Die");
                break;
        }
    }
